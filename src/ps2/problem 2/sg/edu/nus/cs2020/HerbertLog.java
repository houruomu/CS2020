...
public class HerbertLog {
	...
	public int calculateSalary(int begin, int end){
	    int mid = (begin + end) / 2;
	    boolean isMid = true;
	    Record beginRecord = get(begin);
	    Record endRecord = get(end);
	    Record midRecord = new Record("", 0);
	    if(end - begin > 1){
	        midRecord = get(mid);
	    }else{
	        isMid = false;
	    ...
	    if(beginRecord.getName() != getName(begin + 1)){
	        totalSalary += beginRecord.getWages();
	    }
	    
	    if(end == numMinutes() - 1 || (begin != end && endRecord.getName() != getName(end + 1))){
	        totalSalary += endRecord.getWages();
	    }
	    
	    if(isMid && midRecord.getName() != getName(mid + 1)){
	        totalSalary += midRecord.getWages();
	    }
	    
	    if(isMid){
	        if(midRecord.getName() != beginRecord.getName() && mid - begin > 1){
	            totalSalary += calculateSalary(begin + 1, mid - 1);
	        }
	        if(midRecord.getName() != endRecord.getName() && end - mid > 1){
	            totalSalary += calculateSalary(mid + 1, end - 1);
	        }
	    }
	...
	protected int getSalary(int min){
	    return get(min).getWages();
	}
	
	protected String getName(int min){
	    return get(min).getName();
	}
	
	public static void main(String[] args){
		HerbertLog log = new HerbertLog("veryShortNamesHerbert.txt");
        int result = log.calculateSalary(0, (int)log.numMinutes()-1);
